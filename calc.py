...
def Solv(_da, _targ):
    ...
    for _key, _a in [ ('app', _alpha), ('sim', 1.0), ('entr', 0.0)]:
    ...
    print(_da, 'test')
    __test = {}
    for _i in ['app', 'sim', 'entr']:
        for _j in ['app', 'sim', 'entr']:
            if _i != _j:
...
#    global cossim_data, entr_data, eval_data, ratio_sample
    ...
    r_itr = []
    with open('./result_solver.pkl', 'rb') as f:
        r_itr = pickle.load(f)
        
    with open('./result_alphasolve_open2.xls', 'w') as f:
        f.write('DA\t')
        for __ in ['sim', 'entr', 'mid', 'app']:
            f.write(__ +' alpha\t')
            f.write(__ +' MSE\t')
            f.write(__ +' Score_r\t')
            f.write(__ +' Entr_r\t')
 ...
 #                               f.write(str(_[5][(_k, _i, _j,)])+'\t')
        ...
        for _ in r_itr:
            for __ in ['sim', 'entr', 'mid', 'app']:
                ...
                try:
                    _sum[1][__].extend( [ _[1][__] ] * len(_[2]['app']))
                except:
                    _sum[1][__] = [ _[1][__] ] * len(_[2]['app'])

                for i in range(2, 5):
                    try:
                        _sum[i][__].extend(_[i][__])
                    except:
                        _sum[i][__] = _[i][__]

            _sum[5].extend( [ ratio_sample[_[0]] ] * len(_[2]['app']) )

        for __ in ['sim', 'entr', 'mid', 'app']:
            f.write( ''
                            + str(np.mean(_sum[1][__])) + '\t' \
                            + str(np.mean(_sum[2][__])) + '\t' \
                            + str(np.mean(_sum[3][__])) + '\t' \
                            + str(np.mean(_sum[4][__])) + '\t' \
#                            + str(np.mean(_sum[5])) + '\t' \
                            )
        f.write( ''
                            + str(np.mean(_sum[5])) + '\t' \
